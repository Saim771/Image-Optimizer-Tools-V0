...
export default function PdfCompressor() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Simulate page count (in a real app, you'd extract this from the PDF)
  ...
  const handleCompression = async () => {
    ...
    try {
      const result = await compressPdf(originalPdf, compressionLevel)
      setCompressedPdf(result.compressedPdf)
    ...
    <div className="w-full max-w-4xl mx-auto">
      <Card className="mb-8 overflow-hidden border-border/50 shadow-lg">
        <CardContent className="p-0">
          <div
            className="flex flex-col items-center justify-center p-12 cursor-pointer bg-gradient-to-br from-orange-500/5 to-red-500/10 hover:from-orange-500/10 hover:to-red-500/20 transition-all duration-300"
          ...
          >
            ...
            <p className="text-muted-foreground text-center max-w-md">
              Drag and drop your PDF file or click to browse. We support PDF files up to 50MB.
            </p>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <Card className="flex-1 overflow-hidden border-border/50 shadow-md hover:shadow-lg transition-all duration-300">
              <CardContent className="p-6">
                ...
                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-lg bg-muted/50 flex items-center justify-center">
            ...
            <Card className="flex-1 overflow-hidden border-border/50 shadow-md hover:shadow-lg transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold">Compressed PDF</h3>
                </div>
                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-lg bg-muted/50 flex items-center justify-center">
                  {compressedPdf ? (
                    <>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-background/80 backdrop-blur-sm p-3 text-center">
                        <p className="text-sm font-medium">compressed-{fileName}</p>
                        <p className="text-xs text-muted-foreground">{pageCount} pages</p>
                      </div>
                    </>
                  ) : (
                    <div className="absolute inset-0 flex items-center justify-center">
                      {isCompressing ? (
                        <div className="text-center">
                          ...
                          <p className="text-muted-foreground">Optimizing your PDF...</p>
                        </div>
                      ) : (
                        <div className="text-center p-6">
                          <p className="text-muted-foreground mb-2">Compression preview</p>
                          <Button variant="outline" size="sm" onClick={handleCompression} disabled={!originalPdf}>
                            Generate Preview
                          </Button>
                        </div>
                      )}
            ...
            <CardContent className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Button
                  ...
                  disabled={!originalPdf || isCompressing}
                  size="lg"
                  className="w-full"
                >
                  {isCompressing ? (
                    ...
                    <>
                      <Sparkles className="mr-2 h-4 w-4" /> Compress PDF
                    </>
                  )}
                </Button>
                <Button
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mb-4">
                    Your PDF has been successfully compressed with the following results:
                  </p>
